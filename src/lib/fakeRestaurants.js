import { randomNumberBetween, getRandomDateAfter, getRandomDateBefore } from '@/lib/utils.js'
...
export async function generateFakeRestaurantsAndReviews() {
  ...
  for (let i = 0; i < restaurantsToAdd; i++) {
    ...
    for (let j = 0; j < randomNumberBetween(0, 5); j++) {
      const ratingTimestamp = Timestamp.fromDate(getRandomDateAfter(restaurantTimestamp.toDate()))
    ...
    const avgRating = ratingsData.length
      ? ratingsData.reduce((accumulator, currentValue) => accumulator + currentValue.rating, 0) /
        ratingsData.length
      : 0
